...
class Terkey:
  ...
  def about(self):
    self.banner()
    print(f"""
    {a}HAI NGAB WELCOME BACK WITH ME{c}

    INI TOOL BUAT KELEN YANG PENGEN NGASIH TAMBAHAN DI
    KEY DI TERMUX KELEN !
    PROGRAM EDITED BY {a}WahyuWb{c} DAN KARNA UNTUK
    KELEN.
    PROGRAM INI GRATIS AKU AJABDAPETVDI GOOGLE 
    WKWKKWWKWK!

    INI LINK NYA TUH KALO MAU AMBIL AJA DI SITU

    {a}https://wiki.termux.com/wiki/Touch_Keyboard{c}

    INI KLO MAU CHAT TANYAK-TANYAK TAPI GA BELIK
    * Telegram  : {a}https://t.me/wahyuwb{c}
  ...
  def start(self):
    ...
    print(f"    {a}[ {c}MENU {a}]")
    print(f"""
  {a}1.{c} Pake yang biass aja ngab
  {a}2.{c} Kalo mau ngatur sendiri
  {a}3.{c} Tentang
    """
    )
    menu = input(f"  {c}>{a} ")
